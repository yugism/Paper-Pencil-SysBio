...
\[{\rm G6P}\stackrel{v_{\rm in}}{\longrightarrow} {\rm F6P} \stackrel{v_{\rm PFK}}{\longrightarrow} {\rm ADP} \stackrel{v_{\rm out}}{\longrightarrow} \]

G6P、F6P、ADPはそれぞれグルコース6リン酸、フルクトース6リン酸、アデノシン二リン酸を指します。\(v_{in}\)、\(v_{out}\)はそれぞれF6Pの生成速度とADPの分解速度を表します。\(v_{PFK}\)はF6Pを基質とし、
...
\item G6P、F6P、ADPがそれぞれ\(1.0 \times 10^6\)分子だけ存在することをVariable文で定義しなさい。
\item \(v_{in}\)、\(v_{PFK}\)、\(v_{out}\)をそれぞれProcess文で定義しなさい。\(k = 0.2\ {\rm sec}^{-1}\)とします。
\item 上で示した微分方程式(1)(2)は、前問「3」で書いた文で表現できている。では、微分方程式(3)(4)を表現するには、どこをどのように変更すれば良いか答えなさい。
\item emファイルをemlファイルに変換しなさい。
...
\subsection{Sel'kov モデル: 非線形力学系理論と振動現象}
相平面の話題が出てきたところで、これと関係の深い非線形力学系の理論を取り上げます。
非線形力学系の知識体系はシステム生物学において重要度を増していますので、生命情報学科の
諸君にはしっかり習得してもらいたいと思います。\\
\indent さて、比較的早くから見つかっていた生物リズムとして、出芽酵母解
糖系における物質濃度の振動があります。Sel'kovは1968年に
図\ref{fig:09sysbio} に示すモデルを提案し、PFK周辺のフィードバック・ルー
プが振動子の核心であると主張しました。以降、このSel'kovモデルは概日時計
を含む生物リズムを説明する最もシンプルな模型として多くの論文や教科書で
取り上げられています。ここではSel'kovモデルを通じて振動現象の数理を学び、
概日時計との共通点を探ります。

\begin{figure}[ht]
        \centering \includegraphics[height=1.5cm]{sysbio/img/selkov_model.eps}
...
ただし、\(x\)はADP、\(y\)はF6Pの存在量です。これをE-Cellモデルにしたものを次に示します。

\begin{itembox}[l]{{\tt selkov.em}}
  \setlength{\baselineskip}{12pt}
\begin{verbatim}
Stepper ODE45Stepper( DE )
{
...
System System( /CELL/CYTOPLASM )
{
        ...
        Variable Variable( F6P ) { Value 1.0; }
        Variable Variable( ADP ) { Value 1.0; } 
        ...
        Process ExpressionFluxProcess( v0 )
        {
                ...
                Expression "( b )"; 
                VariableReferenceList [ P0 :.:F6P 1 ];
        }
        ...
        Process ExpressionFluxProcess( v1 )
        {
                ...
                Expression "(ここに適切な式を入れなさい) "; 
                VariableReferenceList [ S0 :.:F6P -1 ] [ P0 :.:ADP 1 ];
        }
        ...
        # このモデルでは濃度(MolarConc)ではなく、分子数(Value)で計算している。
                VariableReferenceList [ S0 :.:ADP -1 ];
        }
}
\end{verbatim}
\end{itembox}
...
\subsection{実習課題(4): Sel'kov モデルの解析}
...
\item 空欄を埋めてSel'kov モデルの em ファイルを完成させなさい。Process "\verb+v2+"の例に倣い、\verb+MolarConc+ではなく\verb+Value+を使うこと。a = 0.06、b = 0.6 とする。em ファイルが出来上がったらパラメータ\(a\)、\(b\)を変更し、Hopf 分岐点(「5」で求めた式)を境に振動が生成・消滅することを確認しなさい。
\item (発展)「5」で求めた式に相当する曲線の概形を描きなさい。\(a\)、\(b\)をそれぞれ横軸、縦軸とすること。曲線を境に、どちら側がStableもしくはUnstableなのかも記入しなさい。この項目「7」は必須の課題ではありません。
\end{enumerate}
...
\paragraph{分岐(bifurcation)} パラメータの変化により、固定点の数や種類が変わること。Sel'kovモデルはパラメータの値によって、Spiralの安定性が変わるHopf分岐を示します。
...
\end{itembox}
\ \\
また、後述する発展課題のように、パラメータの値を少しずつ変化させたいような場合は、下の\verb+param.py+の例で示したように\verb+system+関数を使うと良いでしょう。Pythonの\verb+system+関数はカッコ内で指定したコマンドを実行します。\\
...
\subsection{発展: 双安定性}
本実習では生命科学における力学系の例として振動現象を中心に取り上げてき
ましたが、近年注目されているもう一つの力学系の概念を紹介します。細胞内
には、あたかもスイッチのON/OFFかのように、2つの安定状態を持つ系が存在し
ます。このような性質を「双安定性(bistability)」と呼び、力学系の理論では
安定固定点(Stable nodeまたはStable spiral)が2つ存在する状態として説明さ
れます。ここでは双安定性を示す系の例として、Griffithによって提案された、
自己フィードバック・ループを持つ遺伝子発現系のモデルを次に示します。こ
のモデルでは遺伝子XからmRNAが転写され、そのmRNAから翻訳されたタンパク質
が遺伝子X自体の転写を活性化します。Griffithはこれを次のようにモデル化し
ました(ただし、\(x\)はタンパク質、\(y\)はmRNAの存在量)。

\begin{figure}[ht]
        \centering \includegraphics[height=2cm]{sysbio/img/griffith_model.eps}
...
\subsection{発展課題: Griffithモデルの相平面解析}
指定がある問題以外は紙と鉛筆で解きなさい。この発展課題は必須ではありません。
\begin{enumerate}
\item Griffith モデルのヌルクラインを相平面上に描きなさい。
\item 「1」の結果にベクトル場の概略を描き加えなさい。
\item Griffith モデルを線形化し、ヤコビ行列を求めなさい。
\item ヤコビ行列の固有値の符号を調べ、Griffith モデルに現れる固定点を分類しなさい。
\item \(ab < \displaystyle\frac{1}{2}\)、\(ab=\displaystyle\frac{1}{2}\) 、\(ab > \displaystyle\frac{1}{2}\) の3つの場合についてベクトル場を描きなさい。固定点の個数、性質はどのように3つの場合でどのように異なっているか。
\item Sel'kovモデルのemファイルを参考にしてGriffith モデルのemファイルを作成し、E-Cellで動かしなさい。mRNAもしくはタンパク質の量を変化させ、双安定性が見られるかを検討しなさい。物質の量を変化させるには\verb+setProperty+関数を使う。
\end{enumerate}
